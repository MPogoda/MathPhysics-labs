% -*- texinfo -*-
% @deftypefn {Function File} {@var{y} = } task2c (@var{k_x}, @var{v_x}, @var{q_x}, @var{f_x}, @var{mu_0}, @var{mu_1}, @var{xs} )
% @end deftypefn
%
...
% Also it makes even further approximation.
%
% Arguements:
% @table @samp
...
%
% @end table
function [y] = task2c(k_x, v_x, q_x, f_x, mu_0, mu_1, xs)
    h = max( diff( xs ) );
...
% k_{i-0.5} = ( 1/h * int_{x_{i-1}}^{x_i} dx/k(x) )
    ...
    % near y_{i+1}
    As = Rst .* k_plus_s ./ (h^2) .+ (v_plus ./ ks ) .* k_plus_s / h;
    As = -As;
    % near y_i
    Cs = -qs .- Rst .* ((k_minus_s + k_plus_s) ./ (h^2)) .+ (v_minus ./ ks) .* k_minus_s / h .- (v_plus ./ ks) .* k_plus_s / h;
    Cs = -Cs;
    % near y_{i-1}
    Bs = Rst .* k_minus_s ./ (h^2) .- (v_minus ./ ks) .* k_minus_s / h;
    Bs = -Bs;
